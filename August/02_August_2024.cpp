...
public:
    int minSwaps(vector<int>& nums) {
        nfs;
        int sum=accumulate(all(nums) , 0),ans=1e9;
        int sumOnes=0;
        ...
        FOR(i,len(nums)){
            if(i<sum){
                ...
                ans=min(ans,sum - sumOnes);
                sumOnes += (nums[i] - nums[i-sum]);
            }
        }
        return ans;
    }
    ...
    public int minSwaps(int[] nums) {
        ...
        for(int i=0;i<nums1.length;i++){
            if(i<sum){
                ...
                ans=Math.min(ans , sum-sumOnes);
                sumOnes += (nums1[i] - nums1[i-sum]);
            }
        }
        return ans;
    }
...
var minSwaps = function(nums) {
    ...
    for(let i=0;i<nums1.length;i++){
        if(i<sum){
            ...
            ans=parseInt(Math.min(ans , sum-sumOnes));
            sumOnes += (nums1[i] - nums1[i-sum]);
        }
    }
    return ans;

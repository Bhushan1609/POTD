#include <bits/stdc++.h>
#define RunIt cin.tie(NULL)->sync_with_stdio(false);
using namespace std;
using ll=long long;
...
public:
    int minimumPushes(string word) {
        ...
        int iterator=0;
        vd<ll>freq(26,0);
        FOR(i,len(word))freq[word[i]-'a']++;
        sort(all(freq),greater<>());
        FOR(i,26){
            if(i==0 or i%8==0)iterator++;
            buttonPush += (freq[i] * iterator);
        }
        return buttonPush;
    }
};
...
class Solution {
    public int minimumPushes(String word) {
        ...
        Arrays.sort(freq);
        for(int i=0;i<26;i++){
            if((i==0) || (i%8==0))iterator++;
            buttonPush += (freq[26-i-1] * iterator);
        }
        return buttonPush;
    }
}
...
var minimumPushes = function(word) {
    ...
    for(let i=0;i<26;i++){
        if((i==0) || (i%8==0))iterator++;
        buttonPush += parseInt(freq[26-i-1] * iterator);
    }
    return buttonPush;
};
...
public:
    vector<int> nodesBetweenCriticalPoints(ListNode* head) {
        ...
        while(temp and temp->next){
            if((last->val>temp->val and temp->val<temp->next->val) 
            or 
            (last->val<temp->val and temp->val>temp->next->val)){
                v.pb(cnt);
            }
            last=temp;
            temp=temp->next;
        ...
        if(len(v) < 2)return {-1,-1};
        FOR(len(v)-1)ans=min(ans,v[_+1]-v[_]);
...
class Solution {
    public int[] nodesBetweenCriticalPoints(ListNode head) {
        ...
        ListNode last=head;
        ListNode temp=head.next;
        int cnt=2,ans1=Integer.MAX_VALUE;
        while(temp != null && temp.next != null){
            if((last.val>temp.val && temp.val<temp.next.val) 
            || 
            (last.val<temp.val && temp.val>temp.next.val)){
                v.add(cnt);
            }
            last=temp;
            temp=temp.next;
        ...
        for(int i=0;i<v.size()-1;i++)ans1=Math.min(ans1,v.get(i+1)-v.get(i));
        ans[0]=ans1;
        ans[1]=v.get(v.size()-1)-v.get(0);
        return ans;
    }
}
...
var nodesBetweenCriticalPoints = function(head) {
    ...
    for(let i=0;i<v.length-1;i++)ans1=Math.min(ans1,v[i+1]-v[i]);
    ans[0]=ans1;
    ans[1]=v[v.length-1]-v[0];
    return ans;
};
...
public:
    ListNode* mergeNodes(ListNode* head) {
        ...
        while(temp){
            int sum=0;
            ListNode* tempStore=temp;
            while(temp->val != 0){
                sum+=temp->val;
                temp=temp->next;
            }
            tempStore->val=sum;
            tempStore->next=temp->next;
            temp=temp->next;
        }
        return head->next;
    }
...
void yes(bool t = 1) { print(t ? "yes" : "no"); }
void no(bool t = 1) { yes(!t); }
string recursion(Node* root,unordered_map<string,vd<Node*>>&mapp){
    if(!root)return "";
    string left=recursion(root->left,mapp);
    string right=recursion(root->right,mapp);
    string res=to_string(root->data) + ","+left+","+right;
    mapp[res].push_back(root);
    return res;
}
class Solution {
  public:
    vector<Node*> printAllDups(Node* root) {
       ...
       vd<Node*>ans;
       unordered_map<string,vd<Node*>>mapp;
       recursion(root,mapp);
       for(auto i:mapp){
           if(i.second.size()>1){
               ans.pb(i.second[0]);
...
class Solution {
    static String recursion(Node root,HashMap<String,List<Node>>mapp){
        if(root==null)return "";
        String left=recursion(root.left,mapp);
        String right=recursion(root.right,mapp);
        String res=root.data+","+left+","+right;
        mapp.computeIfAbsent(res,k->new ArrayList<>()).add(root);
        return res;
    }
    public List<Node> printAllDups(Node root) {
        List<Node>ans=new ArrayList<>();
        HashMap<String,List<Node>>mapp=new HashMap();
        recursion(root,mapp);
        for(Map.Entry<String,List<Node>>entry:mapp.entrySet()){
